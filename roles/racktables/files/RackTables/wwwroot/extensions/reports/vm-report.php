...
function renderVMReport()
{
    ...
    foreach (scanRealmByText ('object', $sFilter) as $Result)
    {
        ...
        // Container
        $aResult[$Result['id']]['sContainerName'] = 'unknown';
        if ( isset($Result['container_id']) )
            $aResult[$Result['id']]['sContainerName'] = '<a href="'. makeHref ( array( 'page' => 'object', 'object_id' => $Result['container_id']) )  .'">'.$Result['container_name'].'</a>';
    ...
    if ( isset($_GET['csv']) ) {
        ...
        foreach ($aResult as $id => $aRow) {
            ...
            $aCSVRow[5] = $aRow['sOS'];
            $aCSVRow[6] = preg_replace('/<a[^>]*>(.*)<\/a>/iU', '$1', $aRow['sContainerName']);
